 ...
 */
public class MenuSlidingDrawer extends ViewGroup {
    ...
    public MenuSlidingDrawer(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, new int[] {}/*R.styleable.SlidingDrawer*/, defStyle, 0);

        int orientation = a.getInt(0/*R.styleable.SlidingDrawer_orientation*/, ORIENTATION_VERTICAL);
        mVertical = orientation == ORIENTATION_VERTICAL;
        mBottomOffset = (int) a.getDimension(0/*R.styleable.SlidingDrawer_bottomOffset*/, 0.0f);
        mTopOffset = (int) a.getDimension(0/*R.styleable.SlidingDrawer_topOffset*/, 0.0f);
        mAllowSingleTap = a.getBoolean(0/*R.styleable.SlidingDrawer_allowSingleTap*/, true);
        mAnimateOnClick = a.getBoolean(0/*R.styleable.SlidingDrawer_animateOnClick*/, true);

        int handleId = a.getResourceId(0/*R.styleable.SlidingDrawer_handle*/, 0);
        if (handleId == 0) {
            throw new IllegalArgumentException("The handle attribute is required and must refer "
                    + "to a valid child.");
        }

        int contentId = a.getResourceId(0/*R.styleable.SlidingDrawer_content*/, 0);
        if (contentId == 0) {
